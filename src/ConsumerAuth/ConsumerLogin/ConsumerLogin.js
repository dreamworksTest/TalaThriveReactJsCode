...
import { loginConsumerValidation } from "../Validations/Validations";
import { loginConsumerService } from "../Services/Services";
...
const ConsumerLogin = () => {
  ...
  const handleSubmit = async (e) => {
    try {
      e.preventDefault();
      const validate = await loginConsumerValidation(email, password);
      setErrors(validate);
      if (Object.keys(validate).length === 0) {
        ...
        const res = await loginConsumerService(userDetails);
        ...
        if (user.role === "consumer") {
          ...
          if (user.isProfileCompleted === false) {
            navigate("/onBoardConsumer");
          } else {
            localStorage.setItem("isProfileCompleted", user.isProfileCompleted);
            navigate("/consumerAppointment");
          }
        }
      }
    } catch (err) {
          ...
          <div className="d-flex align-items-center logininputwidthmanage">
            <div className="container mainSiteContainer">
              ...
              <p className="loginlinkMobile  d-block d-md-none AthleticsFont">
                ...
                <Link to="/consumerSignup">Sign up now</Link>
              </p>
            </div>
